...
- Whatever commands passed to the server as SSH commands when connecting are passed as arguments to the compiler.
  
  For example, in order to compile a Go project from directory, you must pipe the tarball of the project directory to the SSH connection and use pass the `.` to the server as a command.
...
- Also, just as a warning, if you pass an argument to the server that sets the output file, you will recieve an empty binary, because the server sets the output file name to a random string internally, and your arguments are appended after the (for example) `-o er89jht84wt.bin` argument.

- It should be noted that, until the client is written, you'll still have to make it executable, you can just add `sudo chmod a+x foo` to the end of the chain for this.
